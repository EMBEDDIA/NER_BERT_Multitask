...
class BERT_model_Multitask(BertForTokenClassification):
	...
	def __init__(self, config):
		...
		if hasattr(config, "task_specific_params"):
			...
			self.num_labels_boundaries = len(other_config["labels_boundaries"])+1
			self.classifier_boundaries = nn.Linear(config.hidden_size, self.num_labels_boundaries)
			if other_config["crf"]:
				self.use_crf = True
				...
				crf_constraints = allowed_transitions(other_config["type_crf_constraints"], dict(map(reversed, other_config["labels_boundaries"].items())))
				self.crf_boundaries = ConditionalRandomField(self.num_labels_boundaries, constraints=crf_constraints)
	...
	def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, labels_boundaries=None, tokens_mask=None,
		...
		if labels_boundaries is not None:
			logits_boundaries = self.classifier_boundaries(linear_sequence_output)
